...
roi = edges[int(height / 8):int(3 * height / 4),int(width / 3):int(3 * width / 4)]  # 取图像下半部分
...
for contour in contours:
    ...
    if area > 100:  # 根据实际情况选择合适的面积阈值
        cv2.drawContours(output[int(height / 8):int(3 * height / 4),int(width / 3):int(3 * width / 4)], [contour], -1, (0, 255, 0), 2)
